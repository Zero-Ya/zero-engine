...
namespace ZEngine {
	...
	uint32_t RenderFrame::beginFrame()
	{
		// Wait for fences
	...
	void RenderFrame::middleRecord(uint32_t imageIndex)
	{
		auto& commandBuffer = vk_CommandManager->getCommandBuffers()[frameIndex];
	...
	void RenderFrame::endFrame(uint32_t imageIndex)
	{
		...
		auto result = vk_Ctx->getQueue().presentKHR(presentInfoKHR);
