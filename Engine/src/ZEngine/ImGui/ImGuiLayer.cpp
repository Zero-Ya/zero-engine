...
namespace ZEngine {
	...
	ImGuiKey EngineToImGuiKey(int keycode);
	
	ImGuiLayer::ImGuiLayer() : Layer("ImGuiLayer") {
		Application& app = Application::Get();
		vk_Swapchain = app.getVk_Swapchain();
		frameRenderer = app.getRenderFrame();
		vk_CommandManager = app.getVk_CommandManager();
		imguiUtil = app.getImGuiVulkanUtil();
	}
	...
	void ImGuiLayer::OnRender() {
		auto& imageIndex = Application::Get().currentImageIndex;
		auto& frameIndex = frameRenderer->frameIndex;
	...
	bool ImGuiLayer::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		...
		// Add the main key event
		io.AddKeyEvent(EngineToImGuiKey(e.GetKeyCode()), true);

		// Update modifiers (CRITICAL for InputText shortcuts)
	...
	{
		ImGuiIO& io = ImGui::GetIO();
		io.AddKeyEvent(EngineToImGuiKey(e.GetKeyCode()), false);
		return false;
	}
	...
	//{
	//	ImGuiIO& io = ImGui::GetIO();
	//	io.DisplaySize = ImVec2(static_cast<float>(vk_Swapchain->getExtent().width),
	//		static_cast<float>(vk_Swapchain->getExtent().height));
